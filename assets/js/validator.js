jQuery.fn.validate = function() {
  ...
  var _mensaje = {
    ...
    campo_fecha: 'Esta fecha debe ser mayor a la fecha inicial',
      ...
      if (!obj.prop('readonly') || !obj.prop('disabled')) {
        if ($(this).data('validacion-tipo') != undefined) {
          ...
          $.each($(this).data('validacion-tipo').split('|'), function(i, v) {
            ...
            var label = form_group.find('span');
            ...
            /* Comparamos la fecha con otro control */
            if (v.indexOf('fecha') > -1 && obj.val().length > 0) {
              var _comparacion = true;
              var _aComparar = v.split(':');

              if (_aComparar[1] == 'inicio') {
                _inicio = obj.val();
              }
              if (_aComparar[1] == 'fin') {
                //console.log(obj.val() + ' ' + _inicio);
                ...
                if (obj.val() < _inicio) {
                  ...
                  if (obj.data('validacion-mensaje') == undefined) {
                    small.text(_mensaje.campo_fecha);
                  } else {
                    small.text(obj.data('validacion-mensaje'));
                  }
                }
